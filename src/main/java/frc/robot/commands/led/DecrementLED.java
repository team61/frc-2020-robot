...
public class DecrementLED extends CommandBase {
    ...
    public DecrementLED(LEDSubsystem ledSubsystem, int start, int length, int snakeSize, double time, Color color, int intensityStart, double intensityIncrement) {
    ...
    public DecrementLED(LEDSubsystem ledSubsystem, int start, int length, int snakeSize, double time, Color color) {
        m_ledSubsystem = ledSubsystem;
        m_time = time;
        m_start = start;
        m_snakeSize = snakeSize;
    ...
    public void execute() {
        ...
        if (tail > head) {
            ...
            int intensity = m_intensityStart;
            for (int i = tail; i >= m_start - m_length - 1; i--) {
                m_ledSubsystem.setLEDRGB(i, (int) (r * intensity), (int) (g * intensity), (int) (b * intensity));
                intensity += m_intensityIncrement;
